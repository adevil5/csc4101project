...
class Environment extends Node {
    ...
    //traverse local scope for id's val, move to out environment
    //if not found until built in env would return error
    ...
    //find in current scope if exists, overwrite. Or just add to local scope
    public void define(Node id, Node val) {
        Node definedVal = find(id, scope);
        if (definedVal == null) {
            //define new
            scope = new Cons(new Cons(id, new Cons(val, new Nil())), scope);
        } else {
            //overwrite
            definedVal.setCar(val);
        }
    }

    //look in local scope for var, search outer env if not found. redefine val if found.
